...
import akka.actor._
import domain.command.CommandRoot
...
object CommandHandler {
  ...
  case class PendingCommand(sender: ActorRef, targetProcessorId: String, command: CommandRoot.Command)
}
 ...
 *
 */
trait CommandHandler extends Actor with ActorLogging {
  ...
  /**
   * Processes command.
   * In most cases it should transform message to appropriate aggregate command (and apply some additional logic if needed) and call [[CommandHandler.processAggregateCommand]]
   *
   */
   ...
   */
  def processAggregateCommand(aggregateId: String, command: CommandRoot.Command) = {
